...
export class GroundFogEditor {
    constructor() {
        ...
        // Presets library
        this.stylePresets = {
            'Default':        { intensity: 0.80, opacity: 0.80, heightOffset: 0,  layerSpacing: 15, driftSpeed: 1.0, turbulence: 1.0, nearFade: 10, farFade: 1700, distNear: 80, distFar: 1800, distDensity: 0.90, distAltScale: 1.2, color: '#ffffff' },
            'Morning Mist':   { intensity: 0.50, opacity: 0.90, heightOffset: -5, layerSpacing: 10, driftSpeed: 0.5, turbulence: 0.8, nearFade: 5,  farFade: 1400, distNear: 40, distFar: 1200, distDensity: 1.40, distAltScale: 0.8, color: '#fff5e6' },
            'Deep Forest':    { intensity: 1.30, opacity: 0.85, heightOffset: -10,layerSpacing: 8,  driftSpeed: 0.3, turbulence: 1.5, nearFade: 5,  farFade: 1000, distNear: 25, distFar: 950,  distDensity: 1.60, distAltScale: 0.6, color: '#c8e6c9' },
            'Ethereal Wisps': { intensity: 0.40, opacity: 0.60, heightOffset: 10, layerSpacing: 25, driftSpeed: 2.0, turbulence: 2.5, nearFade: 20, farFade: 1600, distNear: 100, distFar: 1800, distDensity: 0.70, distAltScale: 1.5, color: '#e8d5f5' },
            'Alpine Cloud':   { intensity: 1.00, opacity: 0.95, heightOffset: 18, layerSpacing: 20, driftSpeed: 1.5, turbulence: 1.2, nearFade: 15, farFade: 1800, distNear: 50, distFar: 1300, distDensity: 1.30, distAltScale: 1.0, color: '#e0f0ff' },
            'Desert Mirage':  { intensity: 0.30, opacity: 0.40, heightOffset: -3, layerSpacing: 30, driftSpeed: 3.0, turbulence: 3.0, nearFade: 30, farFade: 1200, distNear: 160, distFar: 2600, distDensity: 0.50, distAltScale: 2.0, color: '#ffe4b5' },
            'Midnight Gloom': { intensity: 1.50, opacity: 0.70, heightOffset: -8, layerSpacing: 12, driftSpeed: 0.4, turbulence: 0.6, nearFade: 5,  farFade: 1500, distNear: 30, distFar: 1050, distDensity: 1.70, distAltScale: 0.5, color: '#7b8fa1' },
            'Vast Horizon':   { intensity: 0.60, opacity: 0.50, heightOffset: 0,  layerSpacing: 20, driftSpeed: 1.0, turbulence: 1.0, nearFade: 20, farFade: 2200, distNear: 200, distFar: 3500, distDensity: 0.60, distAltScale: 2.5, color: '#ffffff' },
        };
...
<div id="gfe-body">
    ...
    <div class="gfe-section">
        <div class="gfe-section-title">Distance Fog (Horizon & Range)</div>
        <div class="gfe-row" title="Distance from player/model where fog starts (0% fog / clear zone)"><label>Start Dist (Clear)</label><input type="range" id="gfe-dist-near" min="0" max="1000" step="10" value="80"><span class="gfe-val" id="gfe-dist-near-val">80m</span></div>
        <div class="gfe-row" title="Distance where fog reaches 100% full density"><label>End Dist (Max)</label><input type="range" id="gfe-dist-far" min="300" max="8000" step="50" value="1800"><span class="gfe-val" id="gfe-dist-far-val">1800m</span></div>
        <div class="gfe-row" title="Overall distance fog density multiplier"><label>Fog Density</label><input type="range" id="gfe-dist-density" min="0.10" max="4.00" step="0.05" value="1.00"><span class="gfe-val" id="gfe-dist-density-val">1.00x</span></div>
        <div class="gfe-row" title="How much the fog distance expands when climbing to high altitudes"><label>Altitude Scale</label><input type="range" id="gfe-dist-alt-scale" min="0.0" max="4.0" step="0.1" value="1.2"><span class="gfe-val" id="gfe-dist-alt-scale-val">1.2x</span></div>
    </div>
    ...
    updateFrame(dt, timePhase = 0) {
        ...
        const targetDistNear = targetCfg.distNear !== undefined ? targetCfg.distNear : 80;
        const targetDistFar = targetCfg.distFar !== undefined ? targetCfg.distFar : 1800;
        const targetDistDensity = targetCfg.distDensity !== undefined ? targetCfg.distDensity : 1.0;
        const targetDistAltScale = targetCfg.distAltScale !== undefined ? targetCfg.distAltScale : 1.2;
